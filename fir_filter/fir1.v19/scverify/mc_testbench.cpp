...
// Generated date: Tue Feb 09 18:46:17 EST 2021
//
// ----------------------------------------------------------------------------
// 
// -------------------------------------
...
bool testbench::shift_reg_ignore;
void mc_testbench_shift_reg_skip(bool v) { testbench::shift_reg_skip = v; }
bool testbench::shift_reg_skip;
bool testbench::shift_reg_skip_quiet;
bool testbench::shift_reg_skip_once;
bool testbench::shift_reg_skip_noerr;
int  testbench::shift_reg_array_comp_first;
int  testbench::shift_reg_array_comp_last;
mc_wait_ctrl testbench::shift_reg_IN_wait_ctrl;
bool testbench::shift_reg_use_mask;
int testbench::shift_reg_output_mask;
mc_wait_ctrl testbench::shift_reg_wait_ctrl;
...
extern "C++" int fir1(int x,  int *shift_reg);
#endif
#ifndef CCS_SCVERIFY_USE_CCS_BLOCK
int testbench::exec_fir1(int x,  int *shift_reg) {
   return mc_testbench::exec_fir1(x, shift_reg);
}
#endif
...
// ============================================
// Function: capture_shift_reg_IN
// --------------------------------------------

void mc_testbench::capture_shift_reg_IN( int *shift_reg)
{
   if (shift_reg_IN_capture_count == wait_cnt)
      wait_on_input_required();
   if (_capture_shift_reg_IN && !testbench::shift_reg_ignore)
   {
      int cur_iter = shift_reg_IN_iteration_count;
      ++shift_reg_IN_iteration_count;
      mgc_sysc_ver_array1D<int,11> shift_reg_IN_tmp;
      int shift_reg_linear_idx = 0;
   shift_reg_access_ptr = shift_reg;
      for (int shift_reg_idx_0 = 0; shift_reg_idx_0 < 11; ++shift_reg_idx_0)
         shift_reg_IN_tmp[shift_reg_linear_idx++] = *(shift_reg_access_ptr++);
      ccs_shift_reg_IN->put(shift_reg_IN_tmp);
      ++shift_reg_IN_capture_count;
      mc_testbench_util::process_wait_ctrl("shift_reg_IN",testbench::shift_reg_IN_wait_ctrl,ccs_wait_ctrl_shift_reg_IN.operator->(),cur_iter,shift_reg_IN_capture_count,0);
   }
   testbench::shift_reg_ignore = false;
}
// ============================================
// Function: capture_shift_reg
// --------------------------------------------

void mc_testbench::capture_shift_reg( int *shift_reg)
{
   if (_capture_shift_reg)
   {
      int cur_iter = shift_reg_iteration_count;
      ++shift_reg_iteration_count;
      mc_golden_info< mgc_sysc_ver_array1D<int,11>, MaskPacket<0, 32> > shift_reg_tmp(testbench::shift_reg_ignore, false, shift_reg_iteration_count);
      shift_reg_tmp._data.mc_testbench_process_array_bounds("shift_reg",testbench::shift_reg_array_comp_first,testbench::shift_reg_array_comp_last,10,0);
      // BEGIN: testbench output_mask control for field_name shift_reg
      if ( testbench::shift_reg_use_mask ) {
         sc_lv<32> tmp_mask_lv;
         type_to_vector(testbench::shift_reg_output_mask, 32, tmp_mask_lv);
         shift_reg_tmp._use_mask = true;
         shift_reg_tmp._packet._mask = tmp_mask_lv;
      }
      // END: testbench output_mask control for field_name shift_reg
      int shift_reg_linear_idx = 0;
   shift_reg_access_ptr = shift_reg;
      for (int shift_reg_idx_0 = 0; shift_reg_idx_0 < 11; ++shift_reg_idx_0)
         shift_reg_tmp._data[shift_reg_linear_idx++] = *(shift_reg_access_ptr++);
      if (!testbench::shift_reg_skip) {
         shift_reg_golden.put(shift_reg_tmp);
         ++shift_reg_capture_count;
      } else {
         if (!testbench::shift_reg_skip_quiet || !testbench::shift_reg_skip_once) {
            std::ostringstream msg; msg.str("");
            msg << "testbench::shift_reg_skip=true for iteration=" << shift_reg_iteration_count << " @ " << sc_time_stamp();
            SC_REPORT_WARNING("User testbench", msg.str().c_str());
            testbench::shift_reg_skip_once = true;
         }
      }
      mc_testbench_util::process_wait_ctrl("shift_reg",testbench::shift_reg_wait_ctrl,ccs_wait_ctrl_shift_reg.operator->(),cur_iter,shift_reg_capture_count,0);
      testbench::shift_reg_use_mask = false;
   }
   testbench::shift_reg_ignore = false;
   testbench::shift_reg_skip = false;
}
// ============================================
...
void mc_testbench::wait_on_input_required()
{
   ...
   while (atleast_one_active_input) {
      ...
      if (_capture_shift_reg_IN && ccs_shift_reg_IN->used() == 0) return;
      that->cpp_testbench_active.write(false);
      if (average_period > SC_ZERO_TIME && sc_time_stamp() != timeout)
         wait(average_period * 10, ccs_x->ok_to_put() | ccs_shift_reg_IN->ok_to_put());
      else
         wait(ccs_x->ok_to_put() | ccs_shift_reg_IN->ok_to_put());
      that->cpp_testbench_active.write(true);
      if (timed_out()) {
...
void mc_testbench::capture_IN(int x,  int *shift_reg)
{
   that->capture_x(x);
   that->capture_shift_reg_IN(shift_reg);
}
// ============================================
// Function: capture_OUT
// --------------------------------------------

void mc_testbench::capture_OUT(int _TOP__return, int x,  int *shift_reg)
{
   that->capture_shift_reg(shift_reg);
...
int mc_testbench::exec_fir1(int x,  int *shift_reg)
{
   #ifndef CCS_SCVERIFY_USE_CCS_BLOCK
   that->cpp_testbench_active.write(true);
   capture_IN(x, shift_reg);
   int _TOP__return = fir1(x, shift_reg);
   ...
   capture_OUT(_TOP__return, x, shift_reg);
   return _TOP__return;
   #else
   int _TOP__return;
   return _TOP__return;
...
void mc_testbench::check_results()
{
   ...
   if (!_capture_shift_reg) {
      cout<<"'shift_reg' - warning, output was optimized away"<<endl;
   } else {
      _num_outputs_checked++;
      _failed |= shift_reg_comp->check_results(shift_reg_capture_count,testbench::shift_reg_skip_noerr);
      _has_capture_counts |= !!(shift_reg_capture_count);
   }
   if (!_capture_return) {
...
void mc_testbench::wait_for_end() {
   ...
   if (shift_reg_comp->get_compare_count() < shift_reg_capture_count) {testbench_end_event.notify(1,SC_NS); return;}
...
void mc_testbench::run() {
   ...
   testbench::shift_reg_ignore = false;
   testbench::shift_reg_skip = false;
   testbench::shift_reg_skip_quiet = false;
   testbench::shift_reg_skip_once = false;
   testbench::shift_reg_skip_noerr = false;
   testbench::shift_reg_array_comp_first = -1;
   testbench::shift_reg_array_comp_last = -1;
   testbench::shift_reg_IN_wait_ctrl.clear();
   shift_reg_IN_capture_count = 0;
   shift_reg_IN_iteration_count = 0;
   testbench::shift_reg_use_mask = false;
   testbench::shift_reg_output_mask = (int) ~0;
   testbench::shift_reg_wait_ctrl.clear();
   shift_reg_capture_count = 0;
   shift_reg_iteration_count = 0;
   shift_reg_IN_access_ptr = 0;
   shift_reg_access_ptr = 0;
   testbench::return_ignore = false;
   testbench::return_skip = false;
   ...
   cout<<"   captured "<<shift_reg_IN_capture_count<<" values of shift_reg_IN"<<endl;
   cout<<"   captured "<<shift_reg_capture_count<<" values of shift_reg"<<endl;
